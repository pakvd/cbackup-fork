...
class Sysinfo
{
    ...
    public static function getJavaVersion()
    {
        ...
        if ($isDocker) {
            // In Docker, Java worker runs in separate container
            // Try to check via worker container or return info message
            $java = SystemHelper::exec('java -version 2>&1');
            if ($java->exitcode) {
                return Yii::t('app', 'Java worker runs in separate Docker container (cbackup_worker)');
            } else {
