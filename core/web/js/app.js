...
$(function() {
    ...
    /** Custom dropdown management - manual control to override Bootstrap */
    
    // Close all navbar dropdowns by default on page load
    $('.navbar .dropdown').removeClass('open');
    $('.navbar [data-toggle="dropdown"]').attr('aria-expanded', 'false');
    
    // Manual dropdown toggle handler - only for navbar dropdowns
    // Use event capture phase to ensure it runs before Bootstrap's handler
    $body.on('click.dropdown.navbar', '.navbar [data-toggle="dropdown"]', function(e) {
        // Only intercept if it's actually in navbar (double-check)
        var $toggle = $(this);
        if (!$toggle.closest('.navbar').length) {
        ...
        // Close all other navbar dropdowns first
        $('.navbar .dropdown.open').not($dropdown).each(function() {
            var $other = $(this);
            $other.removeClass('open');
            $other.find('[data-toggle="dropdown"]').attr('aria-expanded', 'false');
        });
    ...
    $(document).on('click', function(e) {
        ...
        // Don't close if clicking on navbar dropdown toggle (handled above)
        if ($target.closest('.navbar [data-toggle="dropdown"]').length) {
            return;
        }
        
        // Don't close if clicking inside dropdown-menu-static (except navigation links)
        if ($target.closest('.dropdown-menu-static').length) {
            var $link = $target.closest('a[href]');
            if (!$link.length || !$link.attr('href') || $link.attr('href') === '#' || $link.attr('href').indexOf('javascript:') === 0) {
                return; // Don't close static menus on non-navigation clicks
            }
        }
        
        // Close all open navbar dropdowns only
    ...
    /** Close navbar dropdown menu after selecting a navigation item */
    $body.on('click', '.navbar .dropdown-menu > li > a', function(e) {
        var $link = $(this);
        var href = $link.attr('href');
        
        // For static menus, only close on real navigation links
        if ($link.closest('.dropdown-menu-static').length) {
            if (!href || href === '#' || href === 'javascript:void(0);' || href.indexOf('javascript:') === 0) {
                // Check if it's in header or footer
                if ($link.closest('.header, .footer').length) {
                    return; // Don't close on header/footer non-links
                }
            }
        }
        
        // Close the navbar dropdown immediately
        var $dropdown = $link.closest('.dropdown');
        if ($dropdown.closest('.navbar').length) {
            var $toggle = $dropdown.find('[data-toggle="dropdown"]');
    ...
    /** Prevent dropdown-menu-static from closing on clicks inside */
    $('ul.dropdown-menu.dropdown-menu-static').on('click', function (e) {
        var $target = $(e.target);
        // Only prevent if not clicking on a real navigation link
        var $link = $target.closest('a[href]');
        if (!$link.length || !$link.attr('href') || $link.attr('href') === '#' || $link.attr('href').indexOf('javascript:') === 0) {
            if (!$target.closest('.header a[href], .footer a[href]').length) {
                e.stopPropagation();
            }
        }
    });
