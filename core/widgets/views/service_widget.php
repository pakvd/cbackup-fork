...
<table class="table table-no-outer no-margin" id="services_table">
    ...
    ?>
    <tr class="<?= (!$java_server['init'] && !$isDocker) ? 'danger text-muted' : '' ?>">
        <td width="70%">
            <?php
                // In Docker, always show as running (green)
                if ($isDocker) {
                    $class = 'text-success';
                } else {
                ...
                // Only show warning icon if not in Docker and there's an error
                if (!$java_server['init'] && !$isDocker) {
            ...
            <span class="<?= $class ?>">
                <?php
                if ($isDocker) {
                    // In Docker, always show that service runs in separate container
                    echo Yii::t('app', 'running in separate container (cbackup_worker)');
                } else {
                    echo ($service_status) ? Yii::t('app', 'running') : Yii::t('app', 'not running');
                }
                ?>
            </span>
        </td>
    ...
    <tr class="<?= (!$java_scheduler['init'] && !$isDocker) ? 'danger text-muted' : '' ?>">
        <td width="70%">
            <?php
                // In Docker, always show as running (green)
                if ($isDocker) {
                ...
                // Only show warning icon if not in Docker and there's an error
                if (!$java_scheduler['init'] && !$isDocker) {
                    echo Html::tag('span', '<i class="fa fa-warning"></i>', [
                        'class'          => 'text-danger',
                        'data-toggle'    => 'tooltip',
                        'data-placement' => 'left',
            ...
            <span class="<?= $class ?>">
                <?php
                if ($isDocker) {
                    // In Docker, always show that scheduler runs in separate container
                    echo Yii::t('app', 'running in separate container (cbackup_worker)');
                } else {
                    echo ($scheduler_status) ? Yii::t('app', 'running') : Yii::t('app', 'not running');
                }
                ?>
            </span>
        </td>
